            ...
            if( intersect_segment >= 0 )        // TRUE >= 0
            {
              if( dd>0.0 && dd<dmin ) 
              {
                ...
                if( angle_effect >= visualsensorthreshold )
                {
                  ...
                  for( k=0 ; k<nbands ; k++ )
                  {			
                    values[j][k] =  ovalues[intersect_segment][k+2] * angle_effect ;  /* the first two locations 
                             in ovalues[][] are the x,y vertex locations. */
...
/* add metabolic discharge for movement here */
  a->instate->metabolic_charge -= 0.75 * a->instate->metabolic_burn_rate * ( fabs(dfb) + fabs(drl) ) ;
  if( a->instate->metabolic_charge<0.0 )
  { 
    a->instate->metabolic_charge = 0.0 ;
    a->inworld_flag=0 ;
...
WORLD_TYPE *make_world( int index, int maxnagents, int maxnobjects, float xmax, float xmin, float ymax, float ymin )
